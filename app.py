...
# ─────────────────────────────────────────────────────────────────────────────
def render_kl_panel(spot, gflip, cwall, pwall, mpain, vtrig, mwall, mval, ticker=""):
    _tkl = get_theme()
    mc = _tkl["blue"]
    ...
    if ticker == "SPY":
        try:
            _ratio = get_es_spy_ratio(spot)
            _clabel = "ES"
        except Exception:
            pass
    elif ticker == "QQQ":
        try:
            _ratio = get_nq_qqq_ratio(spot)
            _clabel = "NQ"
        except Exception:
            pass

    def _cv(val):
        if _ratio and _clabel and val:
            return f'<span style="font-size:9px;opacity:0.55;margin-left:6px;">≈ {_clabel} {val * _ratio:,.0f}</span>'
        return ""

    rows = [
        (_tkl["t1"],    "Spot",          f"${spot:.2f}",    _cv(spot)),
        (_tkl["t2"],    "Zero Gamma",    f"${gflip:.2f}",   _cv(gflip)),
        (_tkl["green"], "Call Wall",     f"${cwall:.2f}",   _cv(cwall)),
        (_tkl["red"],   "Put Wall",      f"${pwall:.2f}",   _cv(pwall)),
        (_tkl["amber"], "Max Pain",      f"${mpain:.2f}",   _cv(mpain)),
        (_tkl["amber"], "Vol Trigger",   f"${vtrig:.2f}",   _cv(vtrig)),
        (mc,            "Momentum Wall", mw_display,        _cv(mwall) if mwall else ""),
    ]

    html = '<div class="kl-panel"><div class="kl-header">Key Levels</div>'
    for color, name, price, conv in rows:
        html += (
            ...
            f'  </div>'
            f'  <span class="kl-price-val" style="color:{color}">{price}{conv}</span>'
            f'</div>'
        )
    html += '</div>'
    return html
...
def _render_daily_levels(ticker: str, spot: float, df, raw_df, T: dict):
    # ── Conversion ratio: SPY→ES or QQQ→NQ ──────────────────────────────────
    _conv_ratio = None
    _conv_label = None
    if ticker == "SPY":
        try:
            _conv_ratio = get_es_spy_ratio(spot)
            _conv_label = "ES"
        except Exception:
            pass
    elif ticker == "QQQ":
        try:
            _conv_ratio = get_nq_qqq_ratio(spot)
            _conv_label = "NQ"
        except Exception:
            pass

    def _conv_str(price_val):
        """Return e.g. '≈ ES 5234' or '' if no conversion available."""
        if _conv_ratio is None or _conv_label is None:
            return ""
        return f"≈ {_conv_label} {price_val * _conv_ratio:,.0f}"

    def _level_card(label, value, color, sublabel="", conv=""):
        t3 = T["t3"]
        bg1 = T["bg1"]
        line2 = T["line2"]
        conv_line = f'<div style="font-family:JetBrains Mono,monospace;font-size:10px;color:{t3};margin-top:2px;">{conv}</div>' if conv else ""
        sub_line  = f'<div style="font-family:Barlow,sans-serif;font-size:8px;color:{t3};margin-top:3px;">{sublabel}</div>' if sublabel else ""
        return (
            f'<div style="flex:1;min-width:120px;background:{bg1};border:1px solid {line2};'
            f'border-radius:4px;padding:12px 16px;border-top:2px solid {color};">'
            f'<div style="font-family:Barlow,sans-serif;font-size:8px;font-weight:600;'
            f'color:{t3};letter-spacing:2px;text-transform:uppercase;margin-bottom:6px;">{label}</div>'
            f'<div style="font-family:JetBrains Mono,monospace;font-size:18px;font-weight:500;'
            f'color:{color};letter-spacing:-0.5px;">{value}</div>'
            f'{conv_line}{sub_line}'
            f'</div>'
        )
    ...
    def _render_section(data: dict, section: str):
        ...
        cards = ""
        cards += _level_card("Call Wall",    f"${c_wall:.2f}", T["green"],  f"Resistance · {exp_dt}", _conv_str(c_wall))
        cards += _level_card("Put Wall",     f"${p_wall:.2f}", T["red"],    f"Support · {exp_dt}",    _conv_str(p_wall))
        cards += _level_card("Zero Gamma",   f"${gflip:.2f}",  T["t2"],    "Dealer flip line",        _conv_str(gflip))
        if max_pain:
            cards += _level_card("Max Pain", f"${max_pain:.2f}", T["amber"], "Option pain target",    _conv_str(max_pain))
        st.markdown(f'<div style="display:flex;gap:8px;margin-bottom:10px;flex-wrap:wrap;">{cards}</div>', unsafe_allow_html=True)

        range_cards = ""
        range_cards += _level_card("Expected High", f"${exp_hi:.2f}", T["green"], f"Spot + {((exp_hi/spot_v-1)*100):.1f}%", _conv_str(exp_hi))
        range_cards += _level_card("Expected Low",  f"${exp_lo:.2f}", T["red"],   f"Spot − {((1-exp_lo/spot_v)*100):.1f}%", _conv_str(exp_lo))
        ...
        if top_oi:
            ...
            oi_conv = ""
            if _conv_ratio and _conv_label:
                oi_conv = " · ".join([f"{_conv_label} {s * _conv_ratio:,.0f}" for s in top_oi[:3]])
            range_cards += _level_card("High OI Nodes", oi_str, T["violet"], "Top open interest strikes", oi_conv)
        ...
        with info_col:
            st.markdown(
                render_kl_panel(spot_price, gamma_flip, call_wall, put_wall,
                                max_pain, vol_trigger, mom_wall, mom_val, ticker=asset_toggle),
                unsafe_allow_html=True
            )
